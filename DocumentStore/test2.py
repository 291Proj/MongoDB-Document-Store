...
#    READ ME ! ! ! ! ! ! !
#
#   1. Insert and use any parameter that you see fit but let me know before you do so
#   2. If you have questions regarding the implementation of the funstions then ask me (AKIB)
#   3. All the functions are divided into 5 parts corresponding to the 5 specifications
#   4. READ the assignment specifications part carefully
#
############################################################################################################
...
def top_users (num: int) -> list:
    ...
    #results = tweets_collection.find().sort({ 'user.followersCount': -1 }).limit(num) # Query the collection
    pipeline = [
...
def main():
    ...
    connect_dbserver('27017')

    #insert_tweet("Hi Hi")
    #holder = keyword_users('ZUZU')
    #pprint(holder)

    holder = top_users(500000)
